...
TEST(StringDataSource, PeekTest){
    ...
    EXPECT_FALSE(EmptySource.Peek(TempCh));//can't peek from empty source
    EXPECT_EQ(TempCh,'x');//no change
    EXPECT_TRUE(Source1.Peek(TempCh));//can peek
    EXPECT_EQ(TempCh,'H');//first character
    TempCh = 'x';
    EXPECT_TRUE(Source1.Peek(TempCh));//second peek should be the same
    EXPECT_EQ(TempCh,'H');
